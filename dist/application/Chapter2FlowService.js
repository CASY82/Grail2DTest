...
export class Chapter2FlowService {
    ...
    interact(a) {
        if (a.startsWith('wine.shelf')) {
            ...
            return { title: `와인 선반 ${n}`, body: f.solved ? '딸깍. 마지막 VI번 선반 맨 위, 병이 아니라 뱀이 새겨진 작은 나무 상자가 돌아갈 준비를 마쳤다.' : f.reset ? '병이 깨지는 소리와 함께 장치가 처음으로 돌아간다. 위층의 발소리가 빨라진다.' : `딸깍. ${f.progress}/6`, sighting: f.reset };
        }
        if (a.startsWith('civic.name.')) {
            ...
            return { title: '증인의 명패', body: f.solved ? '엘리노어, 아이작, 마사, 토머스. 네 증인의 이름이 순서대로 이어지고, 명부 사이에서 ✠ 진실 조각이 떨어진다.' : f.reset ? '울음이 번지며 순서가 지워진다. 비문이 가리키는 차례부터 다시 짚어야 한다.' : '이름 하나가 비문과 맞물렸다.' };
        }
        switch (a) {
            case 'square.candle':
            ...
            case 'market.ledger':
                ...
                return { title: '잡화점 장부', body: '계산대 위에 펼쳐진 장부의 마지막 페이지에 급히 휘갈긴 글씨가 남아 있다.\n\n‘오늘도 성에서 식량을 받으러 오지 않았다.\n열흘째다.\n앨리가 여관을 닫겠다고 했다.\n성문 열쇠를 내게 맡기려 했지만 나는 거절했다.\n그녀는 그것을 지하 저장고에 감추었다고 했다.\nThe Black Lamb. 와인 선반 뒤.’\n\n“여관 지하 저장고…”', autosave: true };
            case 'alley.mirror':
                this.p.set('hollowSighted');
                return { title: '금 간 거울', body: '거울에 비친 상이 보였다. 그의 뒤, 서너 걸음 떨어진 곳에 사람의 형체가 서 있었다. 회색 피부, 축 늘어진 어깨, 그리고 눈동자 없이 텅 빈 눈. 그것은 아직 완전히 그를 알아차리지 못한 것처럼 보였다. 하지만 곧 그 고개가 천천히 돌아갔다. 정확히 그를 향해.', startPursuit: true };
            case 'house.bar': return { title: '움푹 들어간 나무 문', body: '문 너머에서 낮고 일정한 숨소리가 새어 나온다.\n\n쉬이—\n쉬이—\n\n나무 문이 미세하게 움푹 들어간다.' };
            case 'house.hide':
                this.p.set('hidSuccessfully');
                return { title: '빗장을 지른 문', body: '안으로 몸을 밀어 넣고 문을 닫은 뒤 빗장을 질렀다. 잠시 후, 그 발소리가 집 앞에서 멈췄다. 문 너머에서 숨소리가 들렸다. 쉬이— 쉬이— 입술을 깨물고 눈을 감았다. 피 맛이 났다. 한 시간이 지난 것 같은 일 분 뒤, 발소리는 천천히 멀어졌다.' };
            case 'house.diary':
                this.p.set('diaryRead');
                this.p.set('blackLambClueKnown');
                this.p.addItem('truthCircle');
                return { title: 'T. 모리스의 일기', body: '거실 서랍장 위의 가죽 일기장. 표지에는 ‘T. 모리스’라는 이름이 새겨져 있다. 중반으로 갈수록 글씨가 점점 거칠어진다.\n\n‘성에서 이상한 소리가 들리기 시작했다.\n레지널드 경이 무엇인가를 하고 있다.\n시장은 아무 말도 하지 말라고 했다.\n우리는 그의 땅에 살고 있으니까.’\n\n‘그들이 돌아왔다.\n그러나 그들은 더 이상 그들이 아니었다.\n회색 피부. 빈 눈.\n시장은 그것들을 VIRAX라고 불렀다.\n나는 딸을 지하실에 숨겼다.’\n\n‘앨리는 성문 열쇠를 The Black Lamb의 지하 저장고, 와인 선반 뒤에 숨겼다.\n그 열쇠가 다시 쓰이지 않기를 바란다.’\n\n일기장을 덮어 품속에 넣었다. 표지 안쪽에서 ○ 진실 조각을 얻었다.', autosave: true };
            ...
            case 'wine.hint':
                ...
                return { title: '저장고의 약도', body: '한쪽 탁자 위에 저장고의 약도와 짧은 문장이 적혀 있다.\n\n‘뒤를 따르라.\n북쪽의 별 — V\n남쪽의 그림자 — II\n서쪽의 불 — IV\n동쪽의 물 — I\n중앙의 심장 — III\n봉인된 뱀 — VI’\n\n순서대로 건드리라는 뜻이었다.' };
            case 'wine.serpentBox':
                this.p.set('ironGateKeyTaken');
                this.p.addItem('ironGateKey');
                return { title: '뱀이 새겨진 나무 상자', body: '상자를 돌렸다.\n\n딸깍.\n\n중앙 선반 하나가 뒤로 밀려나며 벽이 열렸다. 그 안에는 주먹만 한 철 열쇠가 있었다. 손잡이 끝에는 블랙우드 가문의 문장이 새겨져 있었다.', autosave: true };
            case 'escape.lamp':
                this.p.set('innFireStarted');
                return { title: '바닥에 내리친 램프', body: '열쇠를 움켜쥐는 순간, 지하 계단 입구에 그림자가 드리워졌다. 그것이 내려와 있었다. 싸울 생각은 없었다. 그는 램프를 바닥에 내리쳤다. 유리가 깨지고 기름이 번졌고, 불길이 바닥을 타고 올랐다. 그것이 비명을 지르는 사이, 저장고의 작은 통풍창이 눈에 들어왔다.', autosave: true };
            ...
            case 'gate2.slot': {
                ...
                if (!this.p.owns('ironGateKey'))
                    return { title: '거대한 외성문', body: '문은 돌과 철로 만들어져 있고, 어른 주먹만 한 열쇠 구멍이 검은 입처럼 벌어져 있다. 숲 오두막에서 얻은 녹슨 열쇠를 꺼내 보았지만, 그 크기는 성문에 비하면 너무 작다.\n\n“다른 열쇠가 필요하다는 뜻이겠지.”', autosave: true };
                if (!['truthTriangle', 'truthCircle', 'truthCross'].every(x => this.p.owns(x)))
                    return { title: '세 진실의 석판', body: `주먹만 한 철 열쇠가 외성문을 밀어냈다. 그 안쪽, 녹청이 낀 청동 문 앞에 허리 높이의 석판이 놓여 있다. 석판 위에는 세 개의 홈이 있다. 삼각형, 원, 그리고 뱀이 감긴 십자. 아직 채우지 못한 홈이 남아 있다.\n\n${Chapter2FlowService.SLAB}`, autosave: true };
                this.p.set('truthSlotLit');
                if (!this.p.has('fourNamesKnown'))
                    return { title: '빛나는 석판', body: `조각들을 홈에 끼우자 석판이 낮게 울었다. 푸른빛이 기호를 따라 번졌고, 연금술의 원이 잠깐 떠올랐다. 그러나 문은 열리지 않았다.\n\n${Chapter2FlowService.SLAB}` };
            ...
            case 'chase2.gate':
                ...
                return { title: 'Blackwood 성문', body: '문이 열리자마자 틈으로 몸을 밀어 넣고, 안쪽 벽에 걸린 체인을 힘껏 당겼다. 문이 다시 닫혔다.\n\n쿠궁.', complete: true };
            default: return { title: '조사', body: '사라진 마을 사람들의 흔적만 남아 있다.' };
        }
    }
    /** 원문 1009~1022: 네 석상은 시청 기둥의 네 비문과 1:1로 대응하고, 마지막에 각 명패에 이름이 새겨진다. */
    statue(title, pose, name, epitaph) {
        if (this.p.has('nameSlotSolved'))
            return { title, body: `${pose} 명패에는 ${name}의 이름이 남았고, 빈 눈구멍에서 작은 빛이 떠오른다.` };
        if (this.p.has('fourNamesKnown'))
            return { title, body: `${pose} 발치의 빈 명패는 시청 기둥의 비문 ‘${epitaph}’와 짝이 맞는다. 여기에 새겨야 할 이름은 ${name}이다.` };
