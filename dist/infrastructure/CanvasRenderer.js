...
export class CanvasRenderer {
    ...
    resize() { this.canvas.width = this.logicalW; this.canvas.height = this.logicalH; }
    render(frame) {
        const c = this.ctx;
    ...
    drawBackground(frame) {
        const c = this.ctx;
        const image = this.assets.getImage(frame.area.backgroundAssetId);
        if (image)
            c.drawImage(image, 0, 0, this.logicalW, this.logicalH);
        else {
            const g = c.createLinearGradient(0, 0, 0, this.logicalH);
            g.addColorStop(0, '#111918');
            g.addColorStop(1, '#090d0c');
            c.fillStyle = g;
    ...
    drawWorld(frame) {
        ...
        if (frame.debug) {
            c.strokeStyle = 'rgba(255,80,80,.7)';
            for (const w of frame.area.walls)
    ...
    drawPlayer(frame) {
        ...
        const image = this.assets.getImage('character.lucas');
        if (image)
            c.drawImage(image, p.position.x - 22, p.position.y - 30, 44, 60);
        else {
            c.fillStyle = '#c7c0ad';
            c.beginPath();
            c.arc(p.position.x, p.position.y - 7, 12, 0, Math.PI * 2);
            c.fill();
    ...
    drawHollow(frame) {
        ...
        if (image)
            ...
            c.fillStyle = '#999b93';
            c.fillRect(h.position.x - 13, h.position.y - 25, 26, 50);
            c.fillStyle = '#0a0a0a';
            c.fillRect(h.position.x - 7, h.position.y - 18, 4, 5);
            c.fillRect(h.position.x + 3, h.position.y - 18, 4, 5);
        }
    }
    drawPursuer(frame) {
        if (!frame.pursuer.active)
            return;
        const c = this.ctx;
        const p = frame.pursuer;
        const image = this.assets.getImage(p.assetId);
        if (image)
            c.drawImage(image, p.position.x - p.size.w / 2 - 3, p.position.y - p.size.h / 2 - 8, p.size.w + 6, p.size.h + 14);
        else {
            c.fillStyle = '#5c2020';
            c.fillRect(p.position.x - p.size.w / 2, p.position.y - p.size.h / 2, p.size.w, p.size.h);
            c.fillStyle = '#160404';
            c.fillRect(p.position.x - 6, p.position.y - p.size.h / 2 + 8, 4, 5);
            c.fillRect(p.position.x + 2, p.position.y - p.size.h / 2 + 8, 4, 5);
        }
    }
    drawDarkness(frame) {
        const c = this.ctx;
        ...
        g.addColorStop(.52, 'rgba(0,0,0,.08)');
        g.addColorStop(1, `rgba(1,4,5,${outerAlpha})`);
        c.fillStyle = g;
        c.fillRect(0, 0, this.logicalW, this.logicalH);
        ...
        if (p.lanternOn) {
            ...
            const glow = c.createRadialGradient(p.position.x, p.position.y, 5, p.position.x, p.position.y, 170);
            glow.addColorStop(0, 'rgba(190,145,77,.12)');
            glow.addColorStop(1, 'rgba(0,0,0,0)');
            c.fillStyle = glow;
            c.fillRect(p.position.x - 170, p.position.y - 170, 340, 340);
            c.restore();
        }
    }
    drawHud(frame) {
        const c = this.ctx;
        c.save();
        c.fillStyle = 'rgba(5,8,8,.72)';
        c.fillRect(22, 20, 560, 108);
        c.strokeStyle = 'rgba(130,160,150,.28)';
        c.strokeRect(22, 20, 560, 108);
        c.fillStyle = '#e4e7e4';
        c.font = '600 17px system-ui';
        c.fillText(frame.area.title, 40, 48);
        c.fillStyle = '#879691';
        c.font = '13px system-ui';
        c.fillText(frame.area.subtitle, 40, 70);
        c.fillStyle = '#cdd4d0';
        c.font = '14px system-ui';
        this.wrapText(`목표: ${frame.objective}`, 40, 96, 520, 19);
        c.fillStyle = 'rgba(5,8,8,.72)';
        ...
        c.fillStyle = '#9aa6a2';
        c.fillText(frame.inventoryText, 160, 686);
        if (frame.prompt) {
            const w = Math.min(640, c.measureText(frame.prompt).width + 46);
            c.fillStyle = 'rgba(0,0,0,.78)';
            c.fillRect(640 - w / 2, 565, w, 42);
            c.fillStyle = '#e8ece9';
            c.textAlign = 'center';
            c.fillText(`[E] ${frame.prompt}`, 640, 591);
            c.textAlign = 'left';
        }
        c.restore();
    }
    drawRain(frame) {
        if (!['rain', 'logging', 'chase'].includes(frame.area.ambience))
            return;
        const c = this.ctx;
        const t = performance.now() / 22;
        c.save();
        c.strokeStyle = 'rgba(180,200,198,.11)';
        c.lineWidth = 1;
        for (let i = 0; i < 65; i++) {
            const x = (i * 197 + t * 3) % 1320 - 20;
            const y = (i * 89 + t * 7) % 760 - 20;
            c.beginPath();
            c.moveTo(x, y);
            c.lineTo(x - 7, y + 18);
            c.stroke();
