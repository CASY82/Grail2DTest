...
export class Chapter3Progress {
  ...
  advanceRitual(beat:number){this.ritualBeatRaw=Math.max(this.ritualBeatRaw,beat);}
  objective():string {
    if(this.has('chapter3Complete')) return 'GRAIL COMPLETE';
    if(this.has('boxOpened')) return '받침대 위에서 벌어지는 일을 끝까지 지켜보라.';
    if(this.has('ritualEntered')) return '등 뒤의 발소리를 확인하라.';
    if(this.has('eleanorConfirmed')) return '실험실 안쪽의 의식실로 들어가라.';
    if(this.has('silenceNoted')) return '푸른 횃불의 복도 끝, 실험실로 내려가라.';
    if(this.has('serviceDoorFound')) return '초상화 아래의 작은 문으로 지하로 내려가라.';
    if(this.has('reginaldEncountered')) return '봉쇄된 성의 지하로 내려가라.';
    if(this.has('journalRead')) return '커튼 그림자 속의 인기척을 확인하라.';
    if(this.has('gateSealed')) return '성의 상층을 탐색하고 2층 집무실의 일지를 읽어라.';
