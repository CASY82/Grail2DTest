...
const puzzle=document.querySelector<HTMLElement>('#puzzle');
if(!canvas||!overlay||!puzzle) throw new Error('Required DOM nodes are missing.');

const assets=new ManifestAssetProvider();
const controller=new GameController(
  {1:chapter1World,2:chapter2World,3:chapter3World},
  new BrowserInput(),
  new CanvasRenderer(canvas,assets),
  assets,
  new WebAudioPort(),
  new ModalView(overlay,puzzle),
  new SaveGameService(new LocalStorageSaveRepository())
);
