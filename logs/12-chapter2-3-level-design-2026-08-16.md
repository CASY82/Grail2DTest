...
3. **비주얼 설명** — 좌측에 그랜드 피아노, 악보가 펼쳐진 채이고 촛불 두어 개가 건반 위를 비춘다. 상단 좌측 아치창으로 차가운 `MOON-COLD` 달빛이 붉은 무늬 벽지 위로 길게 떨어진다. 우측 벽난로 앞 의자에 **창백한 드레스를 입은 마네킹**(`GHOST-LINEN`)이 앉아 있다 — 이 실루엣이 "고개를 돌리는" sighting 연출의 대상이다. 바닥 좌하단에 작은 청록 발광 병(규칙 1)이 놓여 있다. 짙은 붉은 러그, 금박 액자들.
4. **연속성** — 벽난로·액자 재질은 `diningRoom`과 공유.
5. **이미지 생성 프롬프트** — `top-down 2D game background, 1280x720, orthographic floor-plan view of an ornate parlor, a grand piano on the left with open sheet music and two candles lit on its lid, a tall arched window at the upper left letting in cold blue moonlight across red patterned wallpaper, a pale ghostly-dressed mannequin figure seated in a chair by a modest fireplace on the right, a small teal-glowing bottle on the floor lower left, gilt picture frames, dark red ornate rug, painterly horror concept art, palette #d8d2c0 #3d4a58 #7a2e14 #8a6b2e with one #22c4ad accent --no vignette, text, watermark, characters, first-person perspective`
...
3. **비주얼 설명** — 돌로 된 실험실, 중앙에 **하얀 붕대 천으로 감긴 엘리노어의 몸**이 석재 제단/탁자 위에 눕혀 있고, 여러 관·전선이 그녀에게서 천장에 매달린 청록 랜턴 장치로 이어진다(`TEAL-FLAME` 지배, 규칙 확장 2 계속). 우하단 전경에는 **경고처럼 따뜻한 촛불 하나**가 타고 있어 이 방에서 유일하게 난색이 허용되는 지점이 된다(대비를 통해 "이곳에 아직 인간적인 무언가가 남아 있다"는 신호). 배경 우상단에 창백한 금빛으로 은은히 빛나는 조각상/형상이 흐릿하게 보인다(`VIRAX-GOLD`의 첫 등장). 천장에서 덩굴 같은 것이 늘어져 있다.
4. **연속성** — 청록 횃불/랜턴 재질은 `serviceCorridorB1`에서 계승. 우상단의 금빛 형상은 `ritualChamber`의 적금색 안개를 예고.
5. **이미지 생성 프롬프트** — `top-down 2D game background, 1280x720, orthographic floor-plan view of a stone laboratory chamber, a body wrapped in pale bandages lying on a stone altar table at the center, tubes and wires running up to a hanging teal-lit lantern apparatus above, teal ambient light dominating the room, a single warm candle burning in the lower right foreground as the only warm point, a faint pale golden glowing statue barely visible in the upper right background, hanging vine-like growth from the ceiling, painterly horror concept art, palette #3ecab3 #cfc7b8 #ffd79a with one #c9a227 accent --no vignette, text, watermark, characters, first-person perspective`
6. **구현 메모** — 1280×720. `lab.tubes`/`lab.pedestal`/`lab.eleanor` 세 인터랙션 좌표를 제단·빈 받침대·엘리노어 각각에 배치.
...
| id | 위치 | 컨셉아트 근거 | 비주얼 요약 | 이미지 생성 프롬프트 |
|---|---|---|---|---|
| `prop.portrait` | `greatHall` | gr3-castle-upper-board 엘리노어 초상화 (정본) | 금박 액자 속 여성 초상, 촛불 2개가 액자를 밝힘 | `top-down 2D game prop sprite, an oil portrait of a young woman in a gilt frame flanked by two candles, aged varnish, painterly horror concept art, palette #8a6b2e #ffd79a #7a2e14, transparent background, orthographic overhead view --no readable text, vignette, watermark, background` |
| `prop.footprints` | `greatHall` | 없음(추정, GR-1 `prop.footprints` 계승) | 체크 바닥 위 젖은 발자국 한 쌍, 안쪽으로만 | `top-down 2D game prop sprite, a pair of wet bootprints leading inward across a checkered marble floor, no return trail, painterly horror concept art, palette #c9c3b0 #3d4a58, transparent background, orthographic overhead view --no vignette, text, watermark, background` |
...
| `prop.mannequin` / `prop.piano` | `parlor` | gr3-castle-upper-board (정본) | 창백한 드레스 마네킹, 그랜드 피아노 | `top-down 2D game prop sprite, a pale ghostly-dressed mannequin figure seated in an armchair, and separately a grand piano with open sheet music and two lit candles, painterly horror concept art, palette #d8d2c0 #1c1a18 #ffd79a, transparent background, orthographic overhead view --no face detail, vignette, text, watermark, background` |
...
| `prop.glassTubes` / `prop.pedestal` | `laboratoryB2` | gr3-castle-lower-lab-board (정본) | 유리관 장치, 빈 받침대 | `top-down 2D game prop sprite, a cluster of glass specimen tubes connected by cables to an overhead teal lantern apparatus, and separately an empty stone pedestal awaiting an object, painterly horror concept art, palette #3ecab3 #cfc7b8 #232a26, transparent background, orthographic overhead view --no visible body, vignette, text, watermark, background` |
